...
globalThis.APP_CONSTANTS = {
  ...
  UI: {
    ...
    MODULE_INFO: {
      'my-demands': '这里集中管理你发布的家教需求。点击右上角「新建需求」，按引导填好学科或非学科项目、年级、预算、期望时间等，就发布到需求广场供老师查看。发布后，老师对你需求的试课意向会逐条出现在对应需求下方：点开即可看到老师的姓名、擅长科目与报价，决定同意或拒绝。已签约或想下架的需求，可以编辑或删除。需求资料填得越完整，越容易被合适的老师选中。',
      'browse-teachers': '这里是全平台教师列表，学生端默认按与你的匹配度从高到低排列。可用上方筛选按地区、科目、性别、报价等缩小范围；每张卡片展示老师的学校年级、擅长科目、高考成绩、授课方式与报价，右侧的匹配度按钮会列出每个需求的具体得分。点老师姓名可打开完整资料卡（含性格关键词、可授课时间段、评价）。满意的老师可以直接把需求发过去，或先发起会话聊一聊。',
      'my-chats': '与老师/学生一对一沟通的聊天窗口。左侧是会话列表，点开即可收发消息，支持文字、图片和文件，消息约每 4 秒自动刷新，全部交流都会留档。右上角的人头图标可打开对方资料卡；聊天过程中的「+」号可呼出发起签约或起草合同，谈妥后在这里确认签约关系。',
      'my-contracts': '这里是你的合同管理区。在会话里谈妥条件后，起草合同并选择已签约的需求，双方确认、正式签约都在这里完成。每张合同卡显示签约对象、授课方式、时薪与状态，可查看正式合同全文、修改条款；已签合同还可查存证、按需撤销。合同签好即具有法律效力，是双方权益的保障。',
      'notifications': '平台各类提醒的汇总页：老师的试课意向、学生推送的需求、签约与合同进展等都会通知到这里。每条通知可直接看到内容；右上角的「屏蔽系统通知」按钮可以一键过滤平台公告类消息，再点一次恢复。处理过的意向与需求，去「我的需求」里查看具体状态。',
      'account-settings': '账户与外观设置。上半部分是账户信息：查看你的用户名与身份，上传头像、管理已登录的设备（可随时踢掉陌生设备）；下半部分是外观：切换亮色/暗色/跟随系统主题，用「UI大小」滑块整体调整字号、按钮与侧边栏尺寸。页面底部是退出登录（二次确认）和注销账户（需密码确认，注销后数据会被清理）。',
      'about': '平台介绍与帮助。在这里了解平台理念、基本签约流程、安全与隐私说明；往下可以看到版本更新记录。页面下方有反馈入口，遇到 Bug 或有建议可以留给我们，也提供「重温新手引导」按钮，随时再看一遍各模块的用法。',
      'browse-demands': '这里是学生发布的全部家教需求，教师端默认按与你的匹配度从高到低排列。可用筛选按年级、科目、地区、预算等缩小范围；每张需求卡展示学生年级、目标科目、当前成绩、期望时间与预算区间。看中的需求点击「提交试课意向」，系统会先弹出确认（避免误发海投），确认后学生就会收到并决定是否同意。你的档案越完整，匹配度越高、越容易被选中。',
      'resource-share': '教师之间共享教学资源的交流区。顶部可发布图文帖子，分享你的课件、讲义或教学心得；列表可按搜索、排序浏览同行分享的资料，看到有用的可以点赞。发布时注意不要包含他人隐私信息。',
      'edit-profile': '完善你的教师档案，资料越完整匹配度越高、越容易被学生选中。按四个大区填写：基本资料（地区、年级、学校、性别、简介）、学科类资料（擅长科目、高考成绩按毕业省份政策填写、报价区间）、非学科类资料（可教的项目与各自报价）、私密资料（微信等联系方式，仅在你与对方建立会话/签约后展示）。填完点保存，档案即时生效。',
      'admin-stats': '平台运行数据总览。展示注册用户数、需求数、合同数等核心指标，快速把握平台整体活跃情况。',
      'admin-traffic': '站点流量与平均延迟监测。查看访问量与 API 响应时间，及时发现异常波动。',
      'admin-students': '学生账户管理。查看全部学生列表，可封禁或解封违规账户，维护平台秩序。',
      'admin-teachers': '教师账户管理。查看全部教师列表，处理学籍认证审核、封禁/解封违规账户。',
      'admin-demands': '全平台需求管理。查看所有用户发布的需求，可删除违规内容，维护广场秩序。',
      'admin-reviews': '评价审核与删除。用户提交的评价先在这里审核，审核通过的才会公开，违规内容可删除或拒绝。',
      'admin-posts': '资料帖子管理。查看教师共享的资料帖子，删除不合规内容（含隐私/违规信息）。',
      'admin-contracts': '合同管理。查看平台全部合同与状态，必要时移除测试或异常数据。',
      'admin-feedback': '用户反馈处理。查看用户提交的 Bug 与建议，逐条查看详情并标记处理状态。',
    },
