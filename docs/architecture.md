...
后端（src/server/）—— 七层咽喉 + 业务域自持，_worker 不写业务策略
  ...
  router.js            路由分发器；server/db.js = 域拆分后的兼容出口
...
前端（src/client/ + web/）—— constants 数据单源 → core 共享层 → features 领域层 → 壳
  ...
  src/client/features/<域>/  领域层：index.js + actions/render/display（显示映射只在此）
  ...
  src/shared/           config/enums/codes/region-data（前后端共享单源，契约 9）
```
...
业务 SQL 只在 `domains/*/repo.js`。`_worker.js` 除保活（keepD1Warm）外不直接 db.prepare；旧 `server/routes-*.js` 文件也不直接写 SQL（已域拆分）。
...
- **显示逻辑单源**：枚举查名/映射/格式化只在域 display.js（core 或 feature 的 display），出现第二处即违规。
- **登录通路唯一**：ensureAuth（selectPage auth 标记 + 写按钮守卫 + api 401 兜底三处汇入同一函数）。
- **数值单源**：一切裸数字（限流/限额/交互参数/动画时长）只进 `src/shared/config.js` CONFIG 或 `src/server/core/constants`，禁止散落。
...
5. 显示映射（状态→文本/枚举查名）只在 `features/xxx/display.js`。
...
- 校验/限额：只动 `src/shared/config.js` CONFIG 或 `src/server/core/constants`。
