 ...
 * 清该用户过期会话 + 写入新会话一次 db.batch（1 次往返；原子）。
 *
 * 设备去重契约：「设备」=「浏览器档案」而非「登录事件」——带合法 deviceId 时按 (user_id, device_id)
 * UPSERT 复用同一行（session_id 稳定、token_hash/label/expires_at 刷新），同一浏览器反复登录只占一行；
 * 无痕/不同浏览器档案/手机各自一行。无 deviceId 回落旧 INSERT（device_id=''，部分唯一索引不约束）。
 * 语义：一个设备一个活跃会话——同设备重登顶掉旧令牌（另一窗口旧会话 401 重登，符合「设备管理」心智）。
 */
