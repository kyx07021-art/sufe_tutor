...
export async function handleRegister(db, body, req) {
  ...
  try {
    ...
    // 业务冲突 409（用户名已占用），非冲突（D1 故障）上抛 500，不裸返 SERVER_ERROR。
  ...
  try {
    ...
    // Z-1-F7 + Q-2a-F4：绑定失败一律回滚刚建用户（无论 UNIQUE 冲突还是 D1/crypto 故障）——
    // 否则账户已建凭证未绑 = 孤儿账户（Q-2a-F4 抓出）。UNIQUE（并发占用）分流 409；
    // 非冲突（真实故障）回滚后仍上抛 500 保留故障可见性。
