...
export async function handleCreateDemand(db, body, req) {
  ...
  // Q-2c-F5: additional_info 门牌红线审计已由 _worker 全局断点 auditBeforeWrite 统一接管
  // （AUDIT_MAP /api/student/demands → demand.additional_info，POST/PUT 全覆盖），域内不再
  // 重复调用 text-audit（原双审致 DeepSeek 调用翻倍）。合规红线不因字段绕行仍有效。
...
export async function handleResolveIntent(db, intentId, body, req) {
  ...
  // Q-2c-F3: 接受前重查教师 banned/deactivated——封禁/注销教师的待处理意向若被 accept 会建死会话
  // （教师无法登录）。对齐 handlePushDemand 的 teacher.banned 过滤口径。
