...
// ============================================================
// 管理员：合同管理（查看全站合同；仅处理异常数据，正常合同勿操作）
// ============================================================
...
async function loadAdminStats() {
  ...
  try {
    // v1.5.0：待办/存量统计 + 观测指标一次并行拉取（dashboard 只读聚合表，不扫日志明细）
    const [statsData, dashData] = await Promise.all([
      dhGet('/api/admin/stats', { domain: 'admin' }),
      api('/api/admin/dashboard'),
    ]);
    const s = statsData.stats;
    const dash = dashData.dashboard || {};
    const todo = dash.todo || {};
    const m = (dash.metrics && dash.metrics.total) || { requests: 0, errors: 0, slow: 0, limited: 0, avgMs: null };

    // 网安审计 N-14：统计数值本应都是数字，但防御性转义（服务端异常/未来字段改文案时防存储型 XSS）
    const num = x => escHtml(Number(x) || 0);
    const fmtMs = v => UI.ADMIN_DASH_AVG_FMT.replace('{n}', v == null ? '—' : Math.round(v));
    const paths = ((dash.metrics && dash.metrics.topPaths) || []).slice(0, 6);
    const status = ((dash.metrics && dash.metrics.status) || []).slice(0, 5);

    el.innerHTML = `
      <div class="admin-panel glass todo-panel">
        <h3>${UI.ADMIN_TODO_TITLE}</h3>
        <div class="todo-grid">
          ${todoItem('admin-verifications', UI.ADMIN_TODO_VERIFICATIONS, todo.verificationsPending)}
      ...
      <div class="admin-panel glass ops-panel">
        <h3>${UI.PAGE_ADMIN_TRAFFIC_DESC} · ${UI.TRAFFIC_RANGE_24H}</h3>
        <div class="ops-kpi">
          <div class="stat-card glass"><div class="stat-value blue">${num(m.requests)}</div><div class="stat-label">${UI.ADMIN_DASH_24H}</div></div>
          <div class="stat-card glass"><div class="stat-value red">${num(m.errors)}</div><div class="stat-label">${UI.ADMIN_DASH_ERRORS}</div></div>
          <div class="stat-card glass"><div class="stat-value amber">${num(m.slow)}</div><div class="stat-label">${UI.ADMIN_DASH_SLOW}</div></div>
          <div class="stat-card glass"><div class="stat-value amber">${num(m.limited)}</div><div class="stat-label">${UI.ADMIN_DASH_LIMITED}</div></div>
          <div class="stat-card glass"><div class="stat-value">${escHtml(fmtMs(m.avgMs))}</div><div class="stat-label">${UI.ADMIN_DASH_AVG}</div></div>
        </div>
        <div class="ops-detail">
          <div class="ops-block">
            <h4>${UI.ADMIN_DASH_TOP_PATHS}</h4>
            ${paths.length ? paths.map(r => `<div class="ops-row"><code>${escHtml(r.path_group)}</code><span>${num(r.count)}</span></div>`).join('') : `<p class="text-muted">${UI.ADMIN_DASH_EMPTY}</p>`}
          </div>
          <div class="ops-block">
            <h4>${UI.ADMIN_DASH_STATUS}</h4>
            ${status.length ? status.map(r => `<div class="ops-row"><span>${escHtml(r.status_group)}</span><span>${num(r.count)}</span></div>`).join('') : `<p class="text-muted">${UI.ADMIN_DASH_EMPTY}</p>`}
          </div>
        </div>
      </div>
...
function renderAdminContentRow(it) {
  const author = it.author && it.author.username ? escHtml(it.author.username) : UI.DEACTIVATED_USER_PREFIX;
...
function openContentPenaltyModal(type, id) {
  ...
  openModal({
    title: UI.ADMIN_CONTENT_PENALTY_TITLE.replace('{type}', _contentTypeName(t)).replace('{id}', id),
    ...
    body: `<div class="form-group">
        ...
        <input type="text" class="form-input" id="penalty-reason" maxlength="80" placeholder="${UI.ADMIN_PENALTY_REASON_PLACEHOLDER}">
      </div>
      <div class="form-group">
        <label class="form-label">${UI.ADMIN_CONTENT_PENALTY_RULE}</label>
        <input type="text" class="form-input" id="penalty-rule" maxlength="30" placeholder="${UI.ADMIN_PENALTY_RULE_PLACEHOLDER}">
      </div>
      <p class="form-hint">${UI.ADMIN_CONTENT_PENALTY_HINT}</p>`,
...
async function submitContentPenalty(type, id, action) {
  ...
  closeModal();
  confirm({
    title: UI.ADMIN_CONTENT_PENALTY_TITLE.replace('{type}', _contentTypeName(type)).replace('{id}', id),
    message: UI.ADMIN_PENALTY_CONFIRM.replace('{action}', action === 'ban' ? UI.ADMIN_CONTENT_PENALTY_BAN : UI.ADMIN_CONTENT_PENALTY_DELETE),
    needReAuth: true,
...
async function doSubmitContentPenalty(type, id, action, reason, rule, capToken) {
  try {
    ...
    showToast(r.message || UI.ADMIN_PENALTY_DONE, 'success');
    closeModal();
    ...
    d => {
      ...
      return { html: list.map(a => {
        ...
        return `<div class="list-card list-card--teacher glass">
          ...
          <div class="admin-award-meta">${UI.ADMIN_AWARD_TEACHER_LABEL}：${escHtml(a.teacher_username || `#${a.teacher_user_id}`)}</div>
...
function rejectAwardModal(awardId) {
  openModal({
    ...
    body: `<div class="form-group">
        ...
        <input type="text" class="form-input" id="award-reject-note" maxlength="200" placeholder="${UI.ADMIN_AWARD_REJECT_PLACEHOLDER}">
      </div>`,
    footer: `<button type="button" class="btn btn-outline glass glass--pressable" onclick="closeModal()">${UI.BTN_CANCEL}</button>
...
async function loadAdminVerifications() {
  ...
  el.innerHTML = `<div class="empty-state"><span class="loader" role="status" aria-label="${UI.LOADING}"><i></i><i></i><i></i></span></div>`;
  try {
    ...
    el.innerHTML = list.map(v => `<div class="list-card glass verif-card" data-id="${v.id}">
      <div class="verif-head">
        ...
        ${v.verify_type === 'admission' ? `<span class="tag tag-accent glass glass--solid">${escHtml(UI.VERIF_ADMISSION_TAG)}</span>` : ''}
...
function viewAdmissionImage(id) {
  ...
  if (!img) return;
  openModal({
    title: UI.ADMISSION_GATE_TITLE + ' · ' + UI.VERIF_ADMISSION_VIEW_IMG,
    body: `<div class="image-viewer-body"><img src="${escHtml(img)}" alt="${UI.ADMISSION_GATE_TITLE}" style="max-width:100%;max-height:70vh;border-radius:var(--lg-r,12px);"></div>`,
  ...
  return `<div class="verif-form">
    ...
    <div class="verif-grid">
      <div class="form-group"><label class="form-label">${escHtml(UI.CHSI_INFO_SCHOOL)} <span class="req">*</span></label>
        <input type="text" class="form-input" id="verif-school-${v.id}" maxlength="30" placeholder="${UI.VERIF_SCHOOL_PLACEHOLDER}"></div>
      <div class="form-group"><label class="form-label">${escHtml(UI.CHSI_INFO_LEVEL)} <span class="req">*</span></label>
        <input type="text" class="form-input" id="verif-level-${v.id}" maxlength="20" placeholder="${UI.VERIF_LEVEL_PLACEHOLDER}"></div>
      ...
      <div class="form-group"><label class="form-label">${escHtml(UI.CHSI_INFO_STATUS)}</label>
        <input type="text" class="form-input" id="verif-status-${v.id}" maxlength="20" placeholder="${UI.VERIF_STATUS_PLACEHOLDER}"></div>
      <div class="form-group"><label class="form-label">${escHtml(UI.CHSI_INFO_YEAR)}</label>
        <input type="text" class="form-input" id="verif-year-${v.id}" maxlength="10" placeholder="${UI.VERIF_YEAR_PLACEHOLDER}"></div>
    </div>
