...
test('设置页滑块：渲染 min/max/现值；拖动实时更新 --ui-scale、数值标签与 localStorage', async () => {
  ...
  // 拖到上限 120：拖动走 setUiScaleLive——v0.26.5 B3 真实页面分块缩放（用户返工：删预览图 mockup，
  // 原来的组件冻结、整体缩放逻辑是可以的，只是要分三块以保证边缘位置不动）：
  // 拖动期 JS 只写 --ui-preview-scale + html[data-ui-previewing] 门控（真实页面顶栏/侧栏头脚/内容区
  // 由 CSS 分块 transform:scale 消费，合成器只读零重排零重绘）；真实页面 --ui-scale 松手 commit 才落真排版。
  ...
  assert.equal(htmlEl().style.getPropertyValue('--ui-scale'), '', '拖动期不动 --ui-scale（真实页面零重绘，仅分块 transform 预览）');
  assert.equal(htmlEl().style.transform, '', '无整页 transform 缩放');
  assert.equal(htmlEl().dataset.uiPreviewing, undefined, 'rAF 消费前无预览门控（CSS 不挂分块 transform）');
  // 等 rAF 帧消费 → 预览缩放变量 + 门控应用（真实页面分块 transform 预览）
  await tick(30);
  assert.equal(htmlEl().style.getPropertyValue('--ui-preview-scale'), (sliderMax / 100).toFixed(3), 'rAF 帧后 --ui-preview-scale 应用（真实页面分块缩放）');
  assert.equal(htmlEl().dataset.uiPreviewing, '1', '拖动中 html[data-ui-previewing] 门控已挂（CSS 分块 transform 消费）');
  assert.equal(htmlEl().style.getPropertyValue('--ui-scale'), '', '预览期 --ui-scale 仍不动（零真实页面重排）');
  // 松手 commit：撤门控 + 清预览变量 + 落真 --ui-scale（一次性全页排版）+ 落盘
  ...
  assert.equal(htmlEl().dataset.uiPreviewing, undefined, 'commit 后预览门控撤销（CSS 不残留 transform/containing block）');
...
test('B1 滑块：pointer 差分拖动——value 按固定初始几何差分，缩放后同位置稳定（无正反馈）', async () => {
  ...
  await tick(30); // rAF 消费 → 预览缩放变量 + 门控应用（--ui-preview-scale，真实页面分块 transform 预览）
  assert.equal(dom.window.document.documentElement.style.transform, '', '预览走分块 transform 无 html 整页缩放');
  assert.equal(dom.window.document.documentElement.dataset.uiPreviewing, '1', '拖动中 html[data-ui-previewing] 门控已挂');
  assert.notEqual(dom.window.document.documentElement.style.getPropertyValue('--ui-preview-scale'), '', '拖动中预览缩放变量已应用（真实页面零重排）');
  ...
  assert.equal(dom.window.document.documentElement.dataset.uiPreviewing, undefined, 'pointerup 后预览门控撤销（无残留 transform）');
...
test('B1 滑块：input/change 事件兜底仍走预览/落盘（键盘与无障碍路径）', async () => {
  ...
  seedSettingsPage(ctx);
  const max = vm.runInContext('CONFIG.UI_SCALE_MAX', ctx);
  // input（键盘改值）→ 预览缩放变量 + 门控（rAF），真实页面不动、不落盘
  vm.runInContext(`window.SLIDER.value = '${max}'; window.SLIDER.dispatchEvent(new window.Event('input', { bubbles: true }));`, ctx);
  await tick(30);
  assert.equal(dom.window.document.documentElement.style.getPropertyValue('--ui-preview-scale'), (max / 100).toFixed(3), 'input 兜底：预览缩放变量应用');
  assert.equal(dom.window.document.documentElement.dataset.uiPreviewing, '1', 'input 兜底：预览门控已挂');
  ...
  assert.equal(dom.window.document.documentElement.dataset.uiPreviewing, undefined, 'change 兜底：预览门控撤销');
