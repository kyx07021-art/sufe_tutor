 ...
 * fieldcrypto AES-GCM roundtrip + 密钥轮换/无密钥语义（F-06 联系方式加密列）。
 * 注意 bindFieldEnv 重置模块级密钥缓存，同进程内测试串行依赖，勿并行用例。
 */
import test from 'node:test';
import assert from 'node:assert/strict';
import { tokenDigest } from '../server/core.js';
import { bindFieldEnv, encryptField, decryptField } from '../server/fieldcrypto.js';
...
test('字段加解密 roundtrip（密文带 enc:v1: 前缀）', async () => {
  bindFieldEnv({ FIELD_ENC_KEY: KEY_A });
...
test('密钥轮换后的历史密文 → [undecryptable]（绝不空串）', async () => {
  ...
  bindFieldEnv({ FIELD_ENC_KEY: KEY_B }); // 轮换密钥
  assert.equal(await decryptField(enc), '[undecryptable]');
  bindFieldEnv({ FIELD_ENC_KEY: KEY_A }); // 换回原密钥仍可解
  assert.equal(await decryptField(enc), 'wechat:abc');
});

test('取不到密钥（非法 key 使 importKey 失败）：加密退明文、解历史密文标记 [encrypted]', async () => {
  // 本地 secrets.js 必有真密钥（故意保留），无法用空 env 模拟无密钥——用非法 base64 逼 fieldKey() 走 catch → null
  bindFieldEnv({ FIELD_ENC_KEY: '!!not-base64!!' });
