...
test('wizard 逐页校验：P3 缺年级 / P4 缺科目 / P8 缺联系方式 → 拦截；合法流到 P8 出提交按钮', () => {
  ...
  // P8 缺联系方式 → 拦截
  fns.demandWizardValidateStep(8);
...
test('R1/R2：教学目标回填；切非学科 → P5 标题改技能现状 + 成绩行清空 + 技能文本框按项目渲染', () => {
  ...
  // 切回学科 → 标题恢复 + 技能容器隐藏
  fns.switchDemandType({ dataset: { type: 'academic' } });
...
test('R3：完成态集合（done∪visited）驱动进度条，不跟当前停留页', () => {
  ...
  fns.demandWizardGoTo(1);
  doc.getElementById('d-province').value = 'shanghai';
  fns.onDemandProvinceChange();
  fns.demandWizardNext(); // P1 校验通过 → done + 到 P2
  ...
  const { dom: dom2, fns: fns2 } = makeCtx();
  const doc2 = dom2.window.document;
  doc2.getElementById('modal-container').innerHTML = fns2.renderDemandModal(null);
  fns2.initDemandForm('shanghai');
  fns2.prefillDemandForm({ id: 11, province: 'shanghai', student_grade: 'senior1', target_type: 'academic', target_subjects: ['math'], current_scores: [] });
  ...
  fns2.demandWizardGoTo(4); // 翻到 P4 → visited
